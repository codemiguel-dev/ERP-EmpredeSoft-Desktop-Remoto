...
class RegisterFormClient(QMainWindow):
    def __init__(self):
        super(RegisterFormClient, self).__init__()
        loadUi('design/designRegisterclient.ui', self)
        self.init_ui()
    ...
    def register_client(self):
        ...
        typeclient = self.clienttypecombobox.currentText()

        if not rut or not name or not lastname or not address or not phone or not age or not email or not linksocial or not typeclient:
            QMessageBox.warning(self, "Advertencia", "Ambos campos deben ser llenados.")
            return

        self.controller.register_client(rut, name, lastname, address, phone, age, email, linksocial, typeclient)
